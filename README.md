...
The attachments table stores metadata for all files uploaded to the Task API.It ensures each file is associated with a task and (optionally) with the user who uploaded it.

| Column Name      | Type                        | Description                                                | Constraints / Notes                             |
| ---------------- | --------------------------- | ---------------------------------------------------------- | ----------------------------------------------- |
| **id**           | `UUID`                      | Unique identifier for each attachment.                     | Primary key.                                    |
| **task_id**      | `UUID`                      | References the task the attachment belongs to.             | Foreign key → `tasks.id`, `ON DELETE CASCADE`.  |
| **uploader_id**  | `UUID`                      | References the user who uploaded the file (if applicable). | Foreign key → `users.id`, `ON DELETE SET NULL`. |
| **filename**     | `String(512)`               | Original file name uploaded by the user.                   | Required.                                       |
| **content_type** | `String(255)`               | MIME type (e.g., `application/pdf`, `image/png`).          | Optional.                                       |
| **size_bytes**   | `BigInteger`                | File size in bytes.                                        | Optional.                                       |
| **storage_key**  | `String(1024)`              | Path or key for file in storage (local path or S3 key).    | Unique, Required.                               |
| **created_at**   | `TIMESTAMP (timezone=True)` | Timestamp when file was uploaded.                          | Defaults to `now()`.                            |


Relationships:
- task_id → links attachment to its parent task.
- uploader_id → links attachment to the uploading user.
- Deleting a task automatically deletes its attachments (CASCADE).

Purpose:
This schema ensures file metadata is tracked in the database while the actual files are stored in local storage or AWS S3 (depending on environment configuration).
...
**Presigned URLs** are temporary, secure links generated by AWS S3 that let clients directly upload or download files without needing AWS credentials - they expire automatically after a short duration, preventing unauthorized access.
...
Note: ECR (Elastic Container Registry) is where the Docker image is stored - it’s the container image repository. ECS Fargate is the compute service that runs that image as a live container in the cloud. In short, ECR hosts image; ECS Fargate executes it.
...
- **Registered task definition & service:**
   ...
   - Edited `td.json` with
    ...
    The runtimePlatform field ensures the container runs with the correct CPU architecture (x86_64) and operating system (Linux). This prevents compatibility issues when ECS launches tasks on different hardware types.
...
The CloudWatch dashboard (see below) visualizes real-time ECS metrics such as CPUUtilization (blue line) and MemoryUtilization (orange line) for the taskapi-service. These metrics help track resource usage, detect performance bottlenecks, and confirm that the service is stable under load.
